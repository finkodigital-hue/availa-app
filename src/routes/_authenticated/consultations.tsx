...
const STARTER_QUESTIONS: Record<
  "consultation" | "patch_test",
  ...
  consultation: [
    {
      id: "allergies",
      label:
        "Do you have any allergies or known sensitivities relevant to this service?",
      type: "yes_no",
      required: true,
    },
    {
    ...
    },
    {
      ...
      label: "Have you previously reacted to a hair or beauty product?",
      type: "yes_no",
      required: true,
    },
    {
      id: "anything_else",
      label:
        "Is there anything else the salon should know to provide this service safely?",
      type: "long_text",
      required: false,
    },
  ],
  patch_test: [
    {
      id: "previous_reaction",
      label:
        "Have you previously reacted to hair colour, tint, adhesive or another salon product?",
      type: "yes_no",
      required: true,
    },
    {
    ...
    },
    {
      id: "skin_condition",
      label:
        "Do you currently have irritation, broken skin or a skin condition near the test area?",
      type: "yes_no",
      required: true,
    },
  ],
};
const OPTIONAL_STARTER_QUESTION_IDS = new Set([
  "allergy_details",
  "reaction_details",
  "anything_else",
]);

function customerQuestionRequired(
  question: ConsultationQuestion,
  index: number,
  questions: ConsultationQuestion[],
  answers: Record<string, string | boolean>,
) {
  if (!question.required || OPTIONAL_STARTER_QUESTION_IDS.has(question.id))
    return false;
  if (/^if yes\b/i.test(question.label.trim())) {
    const parent = questions
      .slice(0, index)
      .reverse()
      .find((item) => item.type === "yes_no");
    return parent ? answers[parent.id] === true : false;
  }
  return true;
}

function customerQuestionVisible(
  question: ConsultationQuestion,
  index: number,
  questions: ConsultationQuestion[],
  answers: Record<string, string | boolean>,
...
function newEditor(
  kind: "consultation" | "patch_test" = "consultation",
): Editor {
  return {
    name:
      kind === "patch_test" ? "Patch test consent" : "New client consultation",
    description:
      kind === "patch_test"
...
function ConsultationsPage() {
  ...
  if (query.isError) {
    ...
    const missingServerKey = /SUPABASE_SERVICE_ROLE_KEY/i.test(errorMessage);
    const missingDatabaseObjects =
      /relation .* does not exist|schema cache|consultation_(templates|submissions|template_services)/i.test(
        errorMessage,
      );
    const errorTitle = missingServerKey
      ? "Local server connection required"
      : missingDatabaseObjects
        ? "Database update required"
        : "Consultations could not load";
    const errorDescription = missingServerKey
      ? "Add the Supabase server key to the local environment, restart the app, then try again. The live website keeps this key securely on the server."
      : missingDatabaseObjects
        ? "Apply the consultation-forms migration, then try again."
        : "Check the server connection and try loading the consultation workspace again.";
    return (
  ...
  const openTemplate = (template: any) => {
    setEditor({
      ...
      serviceIds: (template.consultation_template_services ?? []).map(
        (link: any) => link.service_id,
      ),
    });
  };
  ...
  const save = async () => {
    ...
    try {
      ...
      toast.success(
        editor.id ? "Consultation form updated" : "Consultation form created",
      );
      setEditor(null);
      await query.refetch();
    } catch (error: any) {
  ...
  const startSigning = async (details: {
    ...
    try {
      const headers = await getServerFnAuthHeaders();
      const started = await startConsultationSubmission({
        data: details,
        headers,
      });
      ...
      if (!record)
        throw new Error(
          "The new client form could not be opened. Refresh and try again.",
        );
  ...
  return (
    <div className="page-wrap max-w-7xl mx-auto px-5 sm:px-8 py-8 sm:py-12">
      <PageHeader
        ...
        action={
          <div className="flex flex-wrap gap-2">
            <Button variant="outline" onClick={() => setEditor(newEditor())}>
              <Plus className="h-4 w-4" />
              New form
            </Button>
            <Button
              onClick={() => setStartingForm(true)}
              disabled={!templates.some((template: any) => template.active)}
            >
      ...
      <button
        ...
        onClick={() => setStartingForm(true)}
        disabled={!templates.some((template: any) => template.active)}
        className="mb-7 w-full rounded-2xl border bg-card p-5 text-left shadow-sm transition hover:border-primary/40 hover:shadow-md disabled:cursor-not-allowed disabled:opacity-60"
      >
        <div className="flex items-center gap-4">
          ...
          <div className="min-w-0 flex-1">
            <div className="font-semibold">Start a form for a customer</div>
            <p className="mt-1 text-sm text-muted-foreground">
              Add their name and contact details, then let them complete and
              sign it here in the salon.
            </p>
          </div>
      ...
      <div className="rounded-xl border border-amber-300/50 bg-amber-50/70 px-4 py-3 text-sm text-amber-950 mb-7 flex gap-3 dark:bg-amber-950/20 dark:text-amber-100">
        ...
        <div>
          <div className="font-semibold">
            Built for UK data-protection safeguards
          </div>
          <p className="mt-0.5 opacity-80">
            Only collect information genuinely needed for the service. Have your
            final form wording, privacy notice and retention policy reviewed by
            a qualified UK professional before launch.
          </p>
        </div>
      </div>
        ...
        <TabsContent value="templates" className="mt-5">
          {query.isLoading ? (
            ...
            <EmptyState
              ...
              description="Start with a salon consultation or patch-test form, then choose which services require it."
              action={
                <Button onClick={() => setEditor(newEditor())}>
              ...
              {templates.map((template: any) => (
                ...
                >
                  ...
                  <div className="mt-5 pt-4 border-t flex flex-wrap gap-x-5 gap-y-2 text-xs text-muted-foreground">
                    ...
                    <span>
                      {template.consultation_template_services?.length ?? 0}{" "}
                      services
                    </span>
      ...
      <StartSigningDialog
        ...
        templates={templates}
        saving={saving}
        onStart={startSigning}
        onClose={() => setStartingForm(false)}
      />
      <RecordDialog
        ...
        onSaved={async () => {
          const refreshed = await query.refetch();
          setSelectedRecord(
            refreshed.data?.submissions.find(
              (record: any) => record.id === selectedRecord?.id,
            ) ?? null,
          );
        }}
      />
...
function TemplateEditor({
  ...
  services: any[];
  saving: boolean;
  onChange: (value: Editor | null) => void;
  onSave: () => void;
  onClose: () => void;
}) {
  ...
  const updateQuestion = (
    index: number,
    patch: Partial<ConsultationQuestion>,
  ) =>
    update({
  ...
  return (
    <Dialog open onOpenChange={(open) => !open && onClose()}>
      <DialogContent className="max-w-3xl max-h-[92vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle className="font-display text-2xl">
            {editor.id ? "Edit consultation form" : "Create consultation form"}
          </DialogTitle>
          <DialogDescription>
            The signed version is frozen for the client. Future edits create a
            new version without changing old records.
          </DialogDescription>
        </DialogHeader>
        <div className="space-y-7 py-2">
          <section className="grid gap-4 sm:grid-cols-2">
            <div className="sm:col-span-2">
              ...
              <Input
                ...
                placeholder="New client consultation"
              />
            </div>
            <div className="sm:col-span-2">
            ...
            <div>
              ...
              <Select
                value={editor.kind}
                onValueChange={(kind: "consultation" | "patch_test") =>
                  update({ kind })
                }
              >
                <SelectTrigger className="mt-1.5">
                ...
                <SelectContent>
                  ...
                  <SelectItem value="patch_test">Patch test</SelectItem>
                </SelectContent>
              </Select>
            </div>
            <div>
            ...
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-xs text-muted-foreground mt-0.5">
                  Only ask for information needed to provide the selected
                  services safely.
                </p>
              </div>
            ...
            <div className="mt-3 space-y-3">
              {editor.questions.map((question, index) => (
                ...
                >
                    ...
                    <div className="grid gap-3 sm:grid-cols-[1fr_150px] flex-1">
                      ...
                      >
                        ...
                        <SelectContent>
                          ...
                          <SelectItem value="select">Choose option</SelectItem>
                        </SelectContent>
                      </Select>
                    </div>
                    <Button
                  ...
                  <label className="mt-3 ml-6 flex items-center gap-2 text-xs text-muted-foreground">
                    <Checkbox
          ...
          <section>
            <h3 className="font-semibold">Explicit health-data consent</h3>
            <p className="text-xs text-muted-foreground mt-0.5 mb-2">
              Keep this separate from marketing permission. The client must
              actively agree before signing.
            </p>
            <Textarea
              rows={5}
              value={editor.consentText}
              onChange={(event) => update({ consentText: event.target.value })}
            />
          </section>

          <section>
            <h3 className="font-semibold">Services requiring this form</h3>
            <div className="mt-3 rounded-xl border divide-y max-h-56 overflow-y-auto">
              {groupedServices.length === 0 ? (
                <p className="p-4 text-sm text-muted-foreground">
                  Add services first, then return to assign this form.
                </p>
              ) : (
                        ...
                        >
                          <Checkbox
                            ...
                            onCheckedChange={(checked) =>
                              update({
                                serviceIds: checked
                                  ? [...editor.serviceIds, service.id]
                                  : editor.serviceIds.filter(
                                      (id) => id !== service.id,
                                    ),
                              })
                            }
          ...
          <label className="rounded-xl border p-4 flex items-center justify-between gap-4">
            <div>
              ...
              <p className="text-xs text-muted-foreground">
                Paused forms are not requested for new bookings.
              </p>
            </div>
            <Switch
...
function RecordsTable({
  ...
  if (!records.length)
    return (
      <EmptyState
        icon={FileSignature}
        title="No client forms yet"
        description="Use Get customer signature to enter their details and open a form for them to sign in the salon."
      />
    );
  return (
    <div className="rounded-2xl border bg-card overflow-hidden">
      <div className="hidden sm:grid grid-cols-[1.3fr_1fr_1fr_150px_30px] gap-4 px-5 py-3 border-b text-[10px] uppercase tracking-wider text-muted-foreground">
        <span>Client</span>
        <span>Form</span>
      ...
      {records.map((record) => {
        ...
        const booking = relationOne(record.bookings);
        const template = relationOne(record.consultation_templates);
        const name =
          template?.name ?? record.template_snapshot?.name ?? "Deleted form";
        return (
          <button
            key={record.id}
            onClick={() => onOpen(record)}
            className="w-full text-left grid gap-2 sm:grid-cols-[1.3fr_1fr_1fr_150px_30px] sm:items-center px-5 py-4 border-b last:border-0 hover:bg-secondary/30"
          >
            <div>
...
function StatusBadge({ status }: { status: string }) {
  ...
  return (
    <Badge className={`border-0 w-fit ${item.className}`}>{item.label}</Badge>
  );
}
...
function RecordDialog({
  ...
  useEffect(() => {
    ...
    setTestedAt(
      record?.patch_tested_at
        ? new Date(record.patch_tested_at).toISOString().slice(0, 10)
        : new Date().toISOString().slice(0, 10),
    );
    ...
    setSignerName(
      record?.signer_name ?? relationOne(record?.customers)?.name ?? "",
    );
    ...
    record?.answers,
    record?.signer_name,
    record?.patch_test_outcome,
    record?.patch_tested_at,
    record?.patch_tested_by,
    record?.staff_notes,
    record?.customers,
  ]);
  ...
  const requiredComplete = questions.every(
    (question, index) =>
      !customerQuestionRequired(question, index, questions, answers) ||
  ...
  const saveOutcome = async () => {
    ...
    try {
      const headers = await getServerFnAuthHeaders();
      await recordPatchTestOutcome({
        data: {
          id: record.id,
          outcome: outcome as "passed" | "failed" | "retest_required",
          testedAt,
          testedBy,
          notes,
        },
      ...
      toast.success(
        "Patch-test details saved · the client can now review and sign",
      );
      await onSaved();
    } catch (error: any) {
  ...
  const sign = async () => {
    ...
    try {
      const headers = await getServerFnAuthHeaders();
      await signConsultationSubmission({
        data: {
          id: record.id,
          answers,
          signerName,
  ...
  const withdraw = async () => {
    const headers = await getServerFnAuthHeaders();
    await withdrawConsultationConsent({
      data: {
        id: record.id,
        reason: "Withdrawn at the client’s request in the salon",
      },
      headers,
    });
      ...
      <DialogContent className="max-w-2xl max-h-[92vh] overflow-y-auto">
        <DialogHeader>
          ...
          <DialogDescription>
            {customer?.name} ·{" "}
            {record.signed_at
              ? `Signed ${new Date(record.signed_at).toLocaleString()}`
              : "Complete with the client in the salon"}
          </DialogDescription>
        </DialogHeader>
          ...
          <section className="space-y-4 rounded-2xl border p-4">
            <div>
              <h3 className="font-semibold">
                1. Record the completed skin test
              </h3>
              <p className="text-xs text-muted-foreground mt-1">
                Staff complete this section before handing the device to the
                client.
              </p>
            </div>
              ...
              <div>
                ...
                <Select value={outcome} onValueChange={setOutcome}>
                  ...
                  <SelectContent>
                    ...
                    <SelectItem value="failed">Reaction — failed</SelectItem>
                    <SelectItem value="retest_required">
                      Retest required
                    </SelectItem>
                  </SelectContent>
                </Select>
        ...
        {pending ? (
          <section
            className={`space-y-5 ${isPatchTest ? "border-t pt-5" : ""}`}
          >
            <div>
              <h3 className="font-semibold">Customer form</h3>
              <p className="text-xs text-muted-foreground mt-1">
                The customer answers the questions, reviews the consent wording
                and signs below on this device.
              </p>
            </div>
            {questions.map((question, index) =>
              customerQuestionVisible(question, index, questions, answers) ? (
                <QuestionField
                  key={question.id}
                  question={{
                    ...question,
                    required: customerQuestionRequired(
                      question,
                      index,
                      questions,
                      answers,
              ...
              <div className="flex gap-3">
                ...
                <div>
                  <div className="text-sm font-semibold">
                    Explicit consent to process health information
                  </div>
            ...
            <div>
              ...
              <p className="text-xs text-muted-foreground mt-1 mb-2">
                The client should draw their own signature below.
              </p>
              <SignaturePad
                key={`${record.id}-${record.patch_tested_at ?? "new"}`}
                onChange={setSignatureData}
              />
            </div>
            {!patchTestReady && (
              <p className="text-xs text-amber-700">
                Save the completed skin-test details before collecting the
                client’s signature.
              </p>
            )}
            {!requiredComplete && (
              <p className="text-xs text-muted-foreground">
                Answer every question marked with an asterisk before signing.
              </p>
            )}
            <Button
              onClick={sign}
          ...
          <div className="space-y-5">
            {isPatchTest && (
              <div className="rounded-xl border p-4">
                ...
                <div className="font-semibold mt-1">
                  {record.patch_test_outcome === "passed"
                    ? "Passed"
                    : record.patch_test_outcome === "failed"
                      ? "Reaction — failed"
                      : "Retest required"}
                </div>
                <div className="text-xs text-muted-foreground mt-1">
                  {record.patch_tested_at
                    ? new Date(record.patch_tested_at).toLocaleDateString()
                    : ""}
                  {record.patch_tested_by
                    ? ` · Completed by ${record.patch_tested_by}`
                    : ""}
                </div>
                {record.staff_notes && (
              ...
              {questions.map((question) => (
                <div key={question.id} className="p-4">
                  ...
                  <div className="mt-1 text-sm font-medium whitespace-pre-wrap">
                    {typeof record.answers?.[question.id] === "boolean"
                      ? record.answers[question.id]
                        ? "Yes"
                        : "No"
                      : record.answers?.[question.id] || "Not answered"}
                  </div>
                </div>
              ))}
            </div>
            <div className="rounded-xl bg-secondary/40 p-4">
              <div className="text-xs font-semibold">
                Explicit consent statement accepted
              </div>
              ...
              <div>
                ...
                <div className="rounded-xl border bg-white p-3">
                  <img
                    ...
                    alt={`Signature of ${record.signer_name}`}
            ...
            <p className="text-[10px] text-muted-foreground">
              This signed version is locked and cannot be edited.
            </p>
          </div>
        ) : null}
        ...
        <DialogFooter className="items-center">
          {record.status === "signed" && (
            <ConfirmDialog
              ...
              title="Mark consent as withdrawn?"
              description="Use this when the client asks the salon to withdraw consent. The signed evidence remains preserved but is clearly marked withdrawn."
