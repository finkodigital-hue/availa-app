...
function ServicesPage() {
  ...
  const defaultQtyFor = (item?: InventoryItem) =>
    isDiscreteUnit(item) ? 1 : 30;
  ...
  const managedCategories = useMemo(() => {
    ...
    return Array.from(
      new Set([...configuredCategories, ...serviceCategories]),
    ).sort((a, b) => a.localeCompare(b));
  }, [configuredCategories, services]);
  const hasUncategorised = (services ?? []).some(
    (service) => !service.category?.trim(),
  );
  const categories = hasUncategorised
    ? [...managedCategories, "Uncategorised"]
    : managedCategories;
  ...
  useEffect(() => {
    ...
    if (editingServiceId && biz?.plan === "studio") {
      ...
      supabase
        ...
        .then(({ data }) => {
          if (!cancelled)
            setLinked(new Set((data ?? []).map((r) => r.staff_id)));
        });
      supabase
        .from("service_recipe_items")
  ...
  const renameCategory = async (original: string) => {
    ...
    setCategorySaving(true);
    const next = managedCategories.map((category) =>
      category === original ? name : category,
    );
    try {
  ...
  const attachStockItem = (item: InventoryItem) => {
    if (!recipe.some((line) => line.inventory_item_id === item.id)) {
      setRecipe([
        ...recipe,
        { inventory_item_id: item.id, quantity: defaultQtyFor(item) },
      ]);
    }
    ...
    try {
      ...
      const { data, error } = await supabase
        ...
        .insert({
          ...
          current_stock: stock,
          low_stock_threshold:
            stock > 0 ? Math.max(1, Math.round(stock * 0.2)) : 0,
        })
        .select(
          "id, name, unit, cost_cents, current_stock, low_stock_threshold",
        )
  ...
  const save = async () => {
    ...
    if (
      hasGap &&
      !(Number(edit.gap_min) > 0 && Number(edit.active_after_min) > 0)
    ) {
      return toast.error(
        "Gap and second segment must both be greater than 0 minutes",
      );
    }
    if (recipe.some((line) => !(Number(line.quantity) > 0))) {
      return toast.error("Stock amount used must be greater than 0");
    }
    if ((edit.aftercare_message?.length ?? 0) > 2000) {
      return toast.error(
        "Aftercare instructions must be 2,000 characters or less",
      );
    }
    const rebookingDays = edit.rebooking_interval_days
      ? Number(edit.rebooking_interval_days)
      : null;
    if (rebookingDays && (rebookingDays < 7 || rebookingDays > 365)) {
      return toast.error("Choose a rebooking interval between 7 and 365 days");
    }
    ...
    try {
      ...
      if (
        ...
        if (!(await saveCategoryNames([...managedCategories, categoryName])))
          return;
      }
      const payload = {
        business_id: bid,
        ...
        aftercare_message: edit.aftercare_message?.trim() || null,
        rebooking_interval_days: rebookingDays,
      };
      ...
      if (linked.size > 0) {
        const { error: staffError } = await supabase
          .from("service_staff")
          .insert(
            Array.from(linked).map((staff_id) => ({
              service_id: sid,
              staff_id,
              business_id: bid,
            })),
          ...
          <section className="flex min-h-[560px] flex-col border-b lg:min-h-0 lg:border-b-0 lg:border-r">
            <div className="space-y-3 border-b p-4">
              ...
              <p className="flex items-center gap-1.5 text-xs text-muted-foreground">
                <Pencil className="h-3.5 w-3.5" /> Click any service below to
                edit it.
              </p>
            </div>
                  ...
                  <div className="mb-6 flex flex-wrap items-start justify-between gap-3">
                    ...
                    {edit.id && recipeCounts?.[edit.id] ? (
                      <span className="inline-flex items-center gap-1 rounded-full bg-secondary px-3 py-1.5 text-xs text-muted-foreground">
                        <Package className="h-3 w-3" /> {recipeCounts[edit.id]}{" "}
                        stock item
                    ...
                    <div className="mb-6 rounded-2xl border bg-card p-4 sm:p-5">
                      <div>
                        ...
                        <p className="mt-1 text-xs leading-5 text-muted-foreground">
                          Give useful care advice after this service and choose
                          when opted-in clients are normally ready to rebook.
                        </p>
                      </div>
                        ...
                        <div>
                          ...
                          <Textarea
                            ...
                            onChange={(event) =>
                              setEdit({
                                ...edit,
                                aftercare_message: event.target.value,
                              })
                            }
                            className="mt-1.5 min-h-28"
                            maxLength={2000}
                            placeholder="For the next 48 hours, avoid…"
                          />
                          <p className="mt-1.5 text-xs text-muted-foreground">
                            Sent by email after the appointment is marked
                            complete. Leave blank to send nothing.
                          </p>
                        </div>
                        <div>
                          ...
                          <div className="mt-1.5 flex items-center gap-2">
                            <Input
                              ...
                              min={7}
                              max={365}
                              value={edit.rebooking_interval_days ?? ""}
                              onChange={(event) =>
                                setEdit({
                                  ...edit,
                                  rebooking_interval_days: event.target.value
                          ...
                          <p className="mt-1.5 text-xs text-muted-foreground">
                            Only emailed to customers who chose salon updates,
                            and never when they already have another booking.
                          </p>
                        </div>
                      </div>
                    </div>
                    ...
                    <div className="rounded-2xl border bg-secondary/50 p-4 sm:p-5">
                      <div className="flex items-start gap-3">
                        ...
                        <div className="min-w-0 flex-1">
                          <h3 className="font-display text-xl">
                            Stock used for each appointment
                          </h3>
                          <p className="mt-1 text-xs leading-5 text-muted-foreground">
                            Optional. Linked stock is deducted automatically
                            when this service is completed.
                          </p>
                        </div>
                      </div>
                          ...
                          {recipe.map((line, index) => {
                            ...
                            const appointmentsLeft =
                              item && line.quantity > 0
                                ? Math.floor(
                                    Number(item.current_stock) / line.quantity,
                                  )
                                : 0;
                            const isLow = item
                              ? Number(item.current_stock) <=
                                  Number(item.low_stock_threshold ?? -1) ||
                                appointmentsLeft <= 3
                              : false;
                            return (
                              <div
                      ...
                      <div className="mt-4 flex flex-wrap items-center gap-3">
                        <Button
                          ...
                          className="bg-card"
                        >
                          <PackagePlus className="mr-2 h-4 w-4" /> Attach stock
                          item
                        </Button>
                    ...
                    <summary className="flex cursor-pointer list-none items-center gap-3 p-4 sm:p-5">
                      ...
                      <div className="min-w-0 flex-1">
                        <h3 className="font-display text-lg">
                          More timing and staff options
                        </h3>
                        <p className="mt-0.5 text-xs text-muted-foreground">
                          Processing time, buffers, calendar colour and who can
                          perform it.
                        </p>
                      </div>
                          ...
                          <div>
                            ...
                            <Input
                              ...
                              onChange={(event) =>
                                setEdit({
                                  ...edit,
                                  gap_min: Number(event.target.value),
                                })
                              }
                          ...
                          <div>
                            ...
                            <Input
                              ...
                              onChange={(event) =>
                                setEdit({
                                  ...edit,
                                  active_after_min: Number(event.target.value),
                                })
                              }
                              className="mt-1.5 h-10"
                            />
                        ...
                        <div>
                          ...
                          <Input
                            ...
                            onChange={(event) =>
                              setEdit({
                                ...edit,
                                buffer_before_min: Number(event.target.value),
                              })
                            }
                            className="mt-1.5 h-10"
                          />
                        ...
                        <div>
                          ...
                          <Input
                            ...
                            onChange={(event) =>
                              setEdit({
                                ...edit,
                                buffer_after_min: Number(event.target.value),
                              })
                            }
                            className="mt-1.5 h-10"
                          />
                  ...
                  {edit.id && (
                    <div className="flex gap-2">
                      <Button
                        ...
                        onClick={() =>
                          toggleArchive(
                            edit as Pick<Service, "id" | "archived_at">,
                          )
                        }
                        className="h-11"
                      >
      ...
      >
        <DialogContent className="max-h-[85vh] overflow-y-auto sm:max-w-lg">
          <DialogHeader>
            ...
            <DialogDescription>
              These are your starting categories. Add your own, rename any of
              them, or delete ones you do not need.
            </DialogDescription>
          </DialogHeader>
      ...
      <Dialog open={stockPickerOpen} onOpenChange={setStockPickerOpen}>
        <DialogContent className="max-h-[85vh] overflow-y-auto sm:max-w-lg">
          <DialogHeader>
            ...
            <DialogDescription>
              Choose an existing stock item, or create one here and attach it
              immediately.
            </DialogDescription>
          </DialogHeader>
            ...
            <div className="grid gap-3 sm:grid-cols-2">
              ...
              <div>
                ...
                <Input
                  ...
                  onChange={(event) =>
                    setNewStockAmount(Number(event.target.value))
                  }
                  className="mt-1.5"
                />
              </div>
