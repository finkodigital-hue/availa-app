...
function ReportsPage() {
  ...
  const {
    ...
    queryKey: ["report-bookings", from.toISOString(), to.toISOString()],
    enabled: !!bid,
    queryFn: () => fetchBookingsInRange(from, to),
  });
  ...
  const {
    ...
    queryKey: ["report-bookings", prevFrom.toISOString(), prevTo.toISOString()],
    enabled: !!bid,
    queryFn: () => fetchBookingsInRange(prevFrom, prevTo),
  });

  const totals = useMemo(() => computeTotals(current ?? []), [current]);
  const prevTotals = useMemo(() => computeTotals(previous ?? []), [previous]);
  const staff = useMemo(
    () => aggregateStaffPerformance(current ?? []),
    [current],
  );
  const services = useMemo(
    () => aggregateServicePerformance(current ?? []),
    [current],
  );
  ...
  return (
    <div className="p-5 sm:p-8 md:p-10 max-w-6xl">
      ...
      {(isError || isPreviousError) && (
        <div className="mb-6 rounded-2xl border border-destructive/25 bg-destructive/5 px-5 py-4 text-sm">
          <p className="font-medium">We couldn’t load your report.</p>
          <p className="mt-1 text-muted-foreground">
            Your figures have not been replaced with zeroes. Try the secure
            report request again.
          </p>
          <button
            type="button"
        ...
        <div className="print:hidden">
          <div className="flex flex-wrap items-center justify-between gap-3 mb-4">
            ...
            <div className="inline-flex rounded-xl border bg-card p-0.5">
              {(
                ["previous_period", "same_period_last_year"] as CompareMode[]
              ).map((m) => (
                <button
                  key={m}
          ...
          {!isLoading && (current?.length ?? 0) === 0 && (
            <p className="text-sm text-muted-foreground mt-4 text-center">
              No bookings in {fmtRange(from, to)} — figures above are zero for
              this range.
            </p>
          )}
        </div>
