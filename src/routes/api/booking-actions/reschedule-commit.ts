  ...
  server: {
    handlers: {
      POST: async ({ request }) => {
        ...
        const lookedUp = await peekBookingActionToken(token, "reschedule");
        ...
        if (result?.ok && lookedUp.ok) {
          try { await processBookingChangeEmails(lookedUp.bookingId); }
          catch { console.error("[reschedule] Customer email queued for cron retry"); }
        }
