  ...
  server: {
    handlers: {
      GET: async ({ request }) => {
        ...
        const [deliveryIssues, changeEmailIssues, paymentIssues, balanceIssues, giftIssues] = await Promise.all([
        ...
        if (deliveryIssues.error || changeEmailIssues.error || paymentIssues.error || balanceIssues.error || giftIssues.error) return Response.json({ status: "error" }, { status: 503, headers: { "cache-control": "no-store" } });
        ...
        const healthy = observed < DEFAULT_THRESHOLD && !unresolvedDeliveries && !unresolvedChangeEmails && !unresolvedPayments && !unresolvedBalanceCheckouts && !unresolvedGiftRefunds;
        return Response.json(
          {
