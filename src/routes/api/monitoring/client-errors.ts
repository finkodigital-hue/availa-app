  ...
  server: {
    handlers: {
      GET: async ({ request }) => {
        ...
        const [deliveryIssues, paymentIssues, balanceIssues, giftIssues] = await Promise.all([
        ...
        if (deliveryIssues.error || paymentIssues.error || balanceIssues.error || giftIssues.error) return Response.json({ status: "error" }, { status: 503, headers: { "cache-control": "no-store" } });
        ...
        const healthy = observed < DEFAULT_THRESHOLD && !unresolvedDeliveries && !unresolvedPayments && !unresolvedBalanceCheckouts && !unresolvedGiftRefunds;
        return Response.json(
          {
