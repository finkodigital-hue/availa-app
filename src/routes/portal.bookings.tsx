    ...
    <div className="animate-rise">
      <div className="mb-8">
        ...
        <p className="text-sm text-muted-foreground mt-2">
          Manage upcoming appointments or review your history. Signed in as{" "}
          {user.email}.
        </p>
      </div>
        ...
        <TabsContent value="past" className="space-y-3">
          ...
          {!isLoading && past.length === 0 && (
            <EmptyState
              ...
              body="Your booking history will live here."
            />
          )}
          {past.map((b) => (
            <BookingCard key={b.id} b={b} canManage={false} />
          ))}
        </TabsContent>
      </Tabs>
            ...
            <DialogDescription>
              {cancelTarget && (
                <>
                  ...
                  with {cancelTarget.businesses?.name} will be cancelled. This
                  can't be undone.
                </>
              )}
            </DialogDescription>
...
function statusBadge(status: string) {
  ...
  return (
    <span
      className={`inline-flex items-center gap-1 text-[11px] uppercase tracking-wider px-2 py-0.5 rounded-full font-medium ${s.cls}`}
    >
...
function BookingCard({
  ...
  const brand = b.businesses
    ? parseTheme(b.businesses.page_theme).colors.primary
    : "hsl(var(--primary))";
    ...
    <div className="group rounded-2xl border bg-card p-5 sm:p-6 transition hover:shadow-elegant animate-rise">
      <div className="grid sm:grid-cols-[auto_1fr_auto] gap-4 sm:gap-6 items-start">
        ...
        <div className="flex sm:flex-col items-stretch gap-2 sm:w-36">
          <div className="text-right font-medium sm:mb-1">
            {fmtMoney(b.price_cents)}
          </div>
          {canManage && canModify && (
            <>
          ...
          {canManage && !canModify && b.status !== "cancelled" && (
            <div className="text-xs text-muted-foreground flex items-start gap-1.5">
              ...
              <span>
                Within the {b.businesses?.cancellation_window_hours ?? 24}h
                window — contact the business.
              </span>
            </div>
          )}
        </div>
...
function RescheduleDialog({
  ...
  const { slots, isLoading } = useAvailableSlots({
    ...
    service: booking?.services
      ? {
          ...
          gap_min: booking.services.gap_min,
          active_after_min: booking.services.active_after_min,
        }
      : undefined,
    date,
  ...
  const days = useMemo(
    () => buildDateStrip(28).slice(stripStart, stripStart + 7),
    [stripStart],
  );
    ...
    try {
      ...
      toast.error(
        e.message?.includes("SLOT_TAKEN")
          ...
          : (e.message ?? "Could not reschedule"),
      );
      ...
      <DialogContent className="max-w-2xl">
        <DialogHeader>
          ...
          <DialogDescription>
            {booking && (
              <>
                {booking.services?.name} with {booking.staff?.name} at{" "}
                {booking.businesses?.name}. Currently:{" "}
        ...
        <div className="space-y-4">
          ...
          <div className="flex items-center gap-2">
            ...
            <Button
              ...
              variant="ghost"
              size="icon"
              onClick={() => setStripStart(Math.min(21, stripStart + 7))}
              disabled={stripStart >= 21}
            >
