...
export function NewBookingDialog({
  ...
  useEffect(() => {
    ...
    (async () => {
      const staffRes = prefill?.staffId
        ? await supabase
            .from("staff")
      ...
      const [svcRes, custRes] = await Promise.all([
        prefill?.serviceId
          ? supabase
              .from("services")
              .select(
                "id, name, duration_minutes, price_cents, buffer_before_min, buffer_after_min, color, gap_min, active_after_min",
              )
        ...
        prefill?.customerId && targetBusinessId === businessId
          ? supabase
              .from("customers")
      ...
      if (cancelled) return;
      let st = staffRes?.data
        ? {
            id: staffRes.data.id,
            name: staffRes.data.name,
            business_id: staffRes.data.business_id,
          }
        : null;
      ...
      if (st && sv) {
        ...
        if (
          links.error ||
          (links.data?.length &&
            !links.data.some((link) => link.staff_id === st!.id))
        )
          st = null;
      }
      if (cancelled) return;
    ...
    try {
      ...
      if (isCustom) {
        const ends_at = new Date(
          new Date(starts_at).getTime() + customDuration * 60000,
        ).toISOString();
      ...
      toast.error(
        e.message?.includes("SLOT_TAKEN")
          ? "That time was just booked — pick another slot."
          : (e.message ?? "Could not create booking"),
      );
    } finally {
      setSubmitting(false);
    }
      ...
      <DialogContent className="sm:max-w-2xl max-h-[90vh] flex flex-col p-5 sm:p-6 gap-3 overflow-hidden">
        <DialogHeader className="shrink-0 space-y-1">
          <DialogTitle className="font-display text-xl flex items-center gap-2">
            {isCustom ? (
              <>
                <Lock className="h-4 w-4" /> Block time / custom
              </>
            ) : (
              <>New booking</>
            )}
          ...
          <DialogDescription className="flex items-center justify-between gap-3">
            <span>
              {isCustom
                ? "Internal only — no customer notification."
                : "We've pre-filled what we already know."}
            </span>
        ...
        <div className="flex-1 min-h-0 overflow-y-auto">
          ...
          {!loadingPrefill && !isCustom && step === "customer" && (
            <CustomerStep
              ...
              initialDraft={newCust}
              crossBusiness={
                !!(staff?.business_id && staff.business_id !== businessId)
              }
              onPick={(c) => {
                setCustomer(c);
                setNewCust(null);
          ...
          {!loadingPrefill &&
            step === "staff" &&
            (isCustom ? customService : service) && (
              <StaffStep
                businessId={staff?.business_id ?? businessId}
                service={(isCustom ? customService : service)!}
                current={staff}
                ...
                onPick={(st) => {
                  ...
                  const next = firstMissing({
                    hasCustomer: isCustom || !!(customer || newCust),
                    hasService: true,
                    hasStaff: true,
                    hasTime: false,
                  });
          ...
          {!loadingPrefill &&
            ...
            (isCustom ? customService : service) && (
              <SlotStep
                ...
                onPick={(iso) => {
                  setTime(iso);
                  setStep(isCustom ? "confirm" : "payment");
                }}
              />
            )}
          ...
          {!loadingPrefill &&
            step === "confirm" &&
            ((isCustom && canConfirmCustom) ||
              (!isCustom && canConfirmRegular)) && (
              <div className="space-y-4">
                  ...
                  <div className="flex items-center justify-between rounded-xl bg-secondary/60 p-3">
                    <div>
                      ...
                      <p className="text-xs text-muted-foreground">
                        Email/SMS the customer when integrations are connected.
                      </p>
                    </div>
                ...
                <DialogFooter>
                  <Button
                    variant="ghost"
                    onClick={() => setStep(isCustom ? "slot" : "payment")}
                  >
                    Back
                  </Button>
...
function Summary({
  ...
  return (
    <div className="rounded-lg border bg-secondary/40 px-2.5 py-1.5 text-sm">
      <SummaryRow
        k={isCustom ? "Title" : "Customer"}
        v={customerOrTitle}
        onEdit={() => onJump(isCustom ? "custom" : "customer")}
      />
      {!isCustom && (
        <SummaryRow
          k="Service"
          v={
            service
              ? `${service.name} · ${service.duration_minutes}m · ${fmtMoney(service.price_cents)}`
              : null
          }
      ...
      {isCustom && service && (
        <SummaryRow
          k="Duration"
          v={`${service.duration_minutes} min`}
          onEdit={() => onJump("custom")}
        />
      )}
      <SummaryRow
        k="Staff"
      ...
      <SummaryRow
        ...
        v={
          time
            ? `${new Date(time).toLocaleDateString([], { weekday: "long", month: "short", day: "numeric" })} · ${fmtTime(time)}`
            : null
        }
        onEdit={() => onJump("slot")}
        last
...
function SummaryRow({
  ...
  return (
    <div
      className={cn(
        "grid grid-cols-[64px_1fr_auto] gap-2 items-center py-1",
    ...
    >
      ...
      <span
        className={`font-medium text-xs truncate ${v ? "" : "text-muted-foreground/60 italic"}`}
      >
        {v ?? "—"}
      </span>
      <button
...
function CustomStep({
  ...
  return (
    <div className="space-y-3">
      ...
      <Button className="w-full" disabled={!title.trim()} onClick={onNext}>
        Continue
      </Button>
    </div>
  );
...
function PaymentStep({
  ...
  setStatus: (v: "unpaid" | "deposit_paid" | "paid") => void;
  onBack: () => void;
  onNext: () => void;
}) {
  return (
    <div className="space-y-3">
      <button
        onClick={onBack}
        className="text-xs text-muted-foreground flex w-fit items-center gap-1"
      >
      ...
      <div className="grid grid-cols-3 gap-2">
        {(["unpaid", "deposit_paid", "paid"] as const).map((v) => (
          <button
            ...
            onClick={() => setStatus(v)}
            className={cn(
              "h-11 rounded-xl border text-xs font-medium capitalize",
              status === v
      ...
      {status === "deposit_paid" && (
        <div>
          ...
          <p className="text-[11px] text-muted-foreground mt-1">
            Currently {fmtMoney(deposit)} of {fmtMoney(price)}.
          </p>
        </div>
      )}
...
function CustomerStep({
  ...
  businessId: string;
  initialDraft?: { name: string; email: string; phone: string } | null;
  crossBusiness?: boolean;
  onPick: (c: Customer) => void;
  onCreate: (c: { name: string; email: string; phone: string }) => void;
}) {
  const [q, setQ] = useState("");
  const [creating, setCreating] = useState(!!crossBusiness || !!initialDraft);
  const [form, setForm] = useState(
    initialDraft ?? { name: "", email: "", phone: "" },
  );
        ...
        <div className="rounded-xl border bg-card divide-y max-h-72 overflow-y-auto">
          ...
          {!isLoading && results?.length === 0 && (
            <div className="p-4 text-sm text-muted-foreground text-center">
              No matches — create new customer above.
            </div>
          )}
          {results?.map((c) => (
            <button
...
function ServiceStep({
  ...
  const { data: lastVisit, isError: historyUnavailable } = useQuery({
    ...
    queryFn: async () => {
      ...
      if (staffResult.error || links.error)
        throw staffResult.error ?? links.error;
      const previousStaff = staffResult.data;
      const eligible =
        previousStaff &&
        (!links.data.length ||
          links.data.some((link) => link.staff_id === previousStaff.id));
      return { ...visit, staff: eligible ? previousStaff : null };
    },
  });
  const previousService = services?.find(
    (item) => item.id === lastVisit?.service_id,
  );
      ...
      {lastVisit && previousService && (
        <div className="rounded-xl border border-primary/20 bg-primary/5 p-3 space-y-2">
          ...
          <p className="text-sm">
            ...
            {lastVisit.staff ? ` with ${lastVisit.staff.name}` : ""}
          </p>
          <p className="text-xs text-muted-foreground">
            {previousService.duration_minutes} min ·{" "}
            {fmtMoney(previousService.price_cents)} at today's price. Choose a
            fresh available time next.
          </p>
          <Button
            type="button"
            variant="outline"
            size="sm"
            onClick={() => onReuse(previousService, lastVisit.staff)}
          >
            {lastVisit.staff
              ? "Use service & stylist"
              : "Use service, choose stylist"}
          </Button>
        </div>
      )}
      {historyUnavailable && (
        <p className="text-xs text-muted-foreground">
          Previous visit unavailable — you can still choose any service below.
        </p>
      )}
      {(services?.length ?? 0) > 5 && (
        <div className="relative">
    ...
    queryFn: async () => {
      ...
      if (!allowAny) {
        ...
        if (linked.data && linked.data.length > 0)
          ids = linked.data.map((r) => r.staff_id);
      }
      let q = supabase
        .from("staff")
        .select("id, name, business_id")
...
function SlotStep({
  ...
  const { slots, isLoading, isError, retry } = useAvailableSlots({
    businessId,
    staffId: staff.id,
    service,
    date,
    searchDays: searchAhead ? 14 : 1,
  });
  const resultDate =
    searchAhead && slots.length ? new Date(slots[0].iso) : date;
  ...
  return (
    <div className="h-full min-h-0 flex flex-col gap-2.5">
      <button
        onClick={onBack}
        className="shrink-0 w-fit text-xs text-muted-foreground inline-flex items-center gap-1"
      >
        <ChevronLeft className="h-3 w-3" /> Back
      </button>
      ...
      <div
        ...
        aria-label="Jump to a booking date"
      >
        {[0, 2, 4, 6, 8].map((weeks) => (
          <Button
            key={weeks}
      ...
      <div className="shrink-0 rounded-xl border border-primary/20 bg-primary/5 p-3 space-y-2">
        <Button
          type="button"
          variant="outline"
          size="sm"
          disabled={isLoading}
          onClick={() => {
            if (searchAhead) void retry();
            else setSearchAhead(true);
          }}
        >
          {isLoading && searchAhead
            ? "Finding availability…"
            : "Find next available"}
        </Button>
        <p className="text-xs text-muted-foreground">
          Checks 14 days from your selected date for {service.name} with{" "}
          {staff.name}. Choose a time below, then review before booking.
        </p>
      </div>
      <p className="text-sm font-medium" aria-live="polite">
        {searchAhead && slots.length > 0 ? "Next available: " : ""}
      ...
      {searchAhead ? (
        <button
          type="button"
          onClick={() => setSearchAhead(false)}
          className="shrink-0 w-fit text-xs font-medium text-primary underline-offset-4 hover:underline"
        >
            ...
            {week.map((d) => {
              ...
              return (
                ...
                >
                  ...
                  <span className="font-display text-sm mt-0.5 tabular-nums">
                    {d.getDate()}
                  </span>
                </button>
              );
      ...
      {isError && (
        <div role="alert" className="text-sm text-destructive">
          Availability couldn't be checked. No times are selectable.{" "}
          <button
            type="button"
            className="underline"
            onClick={() => void retry()}
      ...
      {!isLoading && !isError && slots.length === 0 && (
        <p className="text-sm text-muted-foreground text-center py-6">
          {searchAhead
            ? "No availability in these 14 days. Choose a later start date or another stylist."
        ...
        <div className="flex-1 min-h-0 overflow-y-auto content-start grid grid-cols-3 sm:grid-cols-4 gap-2 pr-0.5">
          {slots.map((s) => (
            <button
              key={s.iso}
              onClick={() => {
                setDate(new Date(s.iso));
                onPick(s.iso);
              }}
              className="h-9 rounded-lg border bg-card hover:bg-primary hover:text-primary-foreground hover:border-transparent text-sm tabular-nums transition-colors"
            >
              {s.time}
            </button>
          ))}
