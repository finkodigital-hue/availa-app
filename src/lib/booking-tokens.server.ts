...
export type BookingAction = "confirm" | "cancel" | "reschedule" | "review";
...
const REVIEW_TOKEN_TTL_AFTER_APPOINTMENT_MS = 90 * 24 * 60 * 60 * 1000;
...
export async function mintBookingActionToken(
  ...
  const ttl =
    action === "review"
      ? REVIEW_TOKEN_TTL_AFTER_APPOINTMENT_MS
      : TOKEN_TTL_AFTER_APPOINTMENT_MS;
  const expires_at = new Date(
    new Date(appointmentStartsAt).getTime() + ttl,
  ).toISOString();
...
// the page.
export async function peekBookingActionToken(
  rawToken: string,
  action: BookingAction,
): Promise<TokenLookup> {
