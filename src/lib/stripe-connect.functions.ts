...
// Kept for already-open clients during deployment. Automatic off-session
// collection is disabled until a verified customer mandate and recovery flow
// exist. A phone/email match alone must never authorise a saved-card charge.
export const takeSavedBalancePayment = createServerFn({ method: "POST" })
  .middleware([requireSupabaseAuth])
  .validator((data: BalanceCheckoutInput) => data)
  .handler(async (): Promise<{ charged: boolean }> => ({ charged: false }));
