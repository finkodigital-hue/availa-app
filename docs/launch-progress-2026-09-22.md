...
- The advisor also shows 62 warnings, including public-callable security-definer functions, mutable function search paths and broad listing of a public-assets bucket. These require function-by-function and policy review; no blanket grant revocation or production policy change was made. Treat the warnings as open launch work, not as proof of exploitation or as resolved findings.
- The project remains on Supabase Free at the founder's request while development continues. Managed database backups, separate Storage-object protection and a restore rehearsal remain pre-launch gates. The 35-day recovery target in the runbook needs an independent archive; a seven-day managed-backup window alone would not meet it.
- A local migration now revokes direct anonymous/authenticated calls to an internal scheduling conflict checker while retaining server access. All 128 application migrations replayed and the 54 full-schema role/security assertions passed. This migration has **not** been applied to production; a live grant check and deployment remain required.

## Controlled booking pilot — 25 September

- PR #80 merged the booking-change email outbox, clearer appointment-SMS notice, internal-helper restriction and booking UX changes into `main`. All four new migrations were applied and recorded in production. The deployed revision is `1368a71`; the public health and 118-page/asset audits passed. A push-triggered uptime check initially raced the deployment, then passed on rerun after the exact revision appeared.
- With explicit founder approval, one additional fictional QA booking was created without a phone number or payment, moved to the following day through a one-use link, then cancelled through a separate one-use link. Resend marked the confirmation, reschedule and cancellation emails delivered; the two private change-outbox events were processed with no manual-review flag. The founder's inbox receipt for these messages has not yet been confirmed. The confirmation email did not include self-service change links; the inaccurate FAQ/help copy was corrected on a separate branch. Twilio's trial restriction remains a launch blocker for SMS.
- The founder visually checked the public QA booking journey through the details step on a real phone and said it looked good. This did not submit a further booking or test accessibility, payment or messaging on that device.

- The existing fictional Studio QA salon was verified to contain only test customer email domains and no required payment. Three guarded, read-only public booking browser checks passed against the live site, including the 390px details step; the guard now permits one verified read-only PostgREST RPC while continuing to block mutations.
- With founder approval for a separate test email and phone, one booking was created through the public page. A database check confirmed exactly one booking and recorded appointment-SMS consent under the original flow. The first confirmation attempt was interrupted by the automation browser closing; one idempotent retry for the same booking returned successfully, and the notification record reached provider-confirmed `delivered`. The founder confirmed the email in their inbox. No card charge was made. The fictional salon's SMS reminder preference was enabled only after confirming no other future booking there had SMS consent; the reminder itself had not run at this point. No personal contact details are stored in this document.
