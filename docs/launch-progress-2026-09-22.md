...
- Follow-up on the separate `codex/launch-email-readiness` branch: reschedule and cancellation events now enter a private, retryable customer-email outbox with an immediate attempt and scheduled backstop. Booking-only SMS reminders are disclosed beside the optional phone field instead of requiring a separate checkbox for new bookings; marketing remains a separate unticked choice. Existing bookings are not backfilled into SMS. These changes passed local migration, launch and build checks, but are not deployed or inbox/SMS-delivery verified. Twilio's trial restriction remains a launch blocker for SMS.
