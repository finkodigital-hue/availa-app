...
- [ ] **Verify monitoring reaches two maintainers** - Test uptime, browser-error, failed-delivery and unresolved-payment alerts, plus the on-call/fallback response.
  - 2026-09-25 evidence: recent production-uptime runs passed. The signed-in log for https://github.com/finkodigital-hue/availa-app/actions/runs/36124259289 confirms the error-alert workflow is correctly reporting **3 notification deliveries marked for manual review** (0 recent browser errors and 0 unresolved payment/balance/gift-refund issues). This is a real operational alert, not a missing secret. An authorised operator must investigate each provider record under `docs/notification-and-usage-operations.md`; do not blindly resend or clear it. Confirm alert notifications reach both maintainers.
