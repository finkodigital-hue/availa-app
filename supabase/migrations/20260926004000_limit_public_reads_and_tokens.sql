-- Bound anonymous reads and one-time-link lookups by a privacy-preserving,
-- daily rotating source key. These quotas are deliberately generous enough
-- for shared household/business connections while limiting automated database
-- and signed-asset amplification.
...
begin
  ...
  case p_scope
    ...
    when 'public_read' then minute_limit := 180; day_limit := 5000;
    when 'token' then minute_limit := 60; day_limit := 1000;
