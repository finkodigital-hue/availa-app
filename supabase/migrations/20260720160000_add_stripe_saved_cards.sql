-- Stripe is the only system that stores card details. Bookzenvo keeps the
-- connected-account customer ID so a business can request a future charge.
ALTER TABLE public.customers
  ADD COLUMN IF NOT EXISTS stripe_customer_id text;

CREATE UNIQUE INDEX IF NOT EXISTS customers_business_stripe_customer_unique
  ON public.customers (business_id, stripe_customer_id)
  WHERE stripe_customer_id IS NOT NULL;

-- Replace the checkout fulfiller with a version that records the Stripe
-- customer created by Checkout alongside the local customer record.
...
REVOKE ALL ON FUNCTION public.fulfill_stripe_checkout(uuid,uuid,uuid,text,text,text,timestamptz,timestamptz,text,text,integer,text,text,text,text) FROM PUBLIC;
REVOKE ALL ON FUNCTION public.fulfill_stripe_checkout(uuid,uuid,uuid,text,text,text,timestamptz,timestamptz,text,text,integer,text,text,text,text) FROM anon, authenticated;
GRANT EXECUTE ON FUNCTION public.fulfill_stripe_checkout(uuid,uuid,uuid,text,text,text,timestamptz,timestamptz,text,text,integer,text,text,text,text) TO service_role;
